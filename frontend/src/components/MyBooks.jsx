          ...
          <ul style={{ listStyleType: 'none', padding: '0' }} key={index}>
            {books.books.map((book) => (
              <li key={book.book_id} style={{ border: '1px solid #ccc', borderRadius: '5px', padding: '10px', marginBottom: '10px' }}>
                ...
                <p>Check-out Date: {new Date(book.checkin_time).toLocaleString()}</p>
                <p>Check-out Location: {books.location_name}</p>
                {book.checkout_time !== '' ? (
                <>
                  <p>Late Fee: $0</p>
                  <p>Returned Date: {new Date(book.checkout_time).toLocaleString()}</p>
                  <p>Return Location: {book.returned_location}</p>
                </>) : (
                <>
